...
class com.adgamewonderland.agw.math.Rectangle {
	...
	/**
	 * prueft, ob uebergebener punkt innerhalb der flaeche des rechtecks liegt
	 * @param point Point punkt, der geprueft werden soll
	 * @return gibt true zurueck, wenn uebergebener punkt innerhalb der flaeche des rechtecks liegt, sonst false
	 */
	public function isPointInside(point:Point ):Boolean
	{
		// boolscher wert, ob der uebergebene punkt innerhalb des rechtecks liegt
		var inside:Boolean = false;
		// anzahl der dimensionen, in denen der punkt innerhalb des rechtecks liegt
		var numdims:Number = 0;
		// testen in x-richtung
		if (point.x >= myPos.x && point.x <= myPos.x + myDims.width) numdims ++;
		// testen in y-richtung
		if (point.y >= myPos.y && point.y <= myPos.y + myDims.height) numdims ++;
		// zurueck geben
		return (inside = (numdims == 2));
	}

	/**
	 * gibt einen eckpunkt des rechtecks zurueck
	 * @param index Numer index des punktes von 0...3
	 * @return gibt den punkt zurueck
	 */
	public function getCorner(index:Number ):Point
	{
		// ecke zurueck geben
		return this.myCorners[index];
	}

	/**
	 * gibt breite des rechtecks (x-abstand zwischen linker oberer und rechter oberer ecke) zurueck
	 * @return gibt breite des rechtecks zurueck
	 */
	public function getWidth():Number
	{
		// x-position linke obere ecke
		var x1:Number = getCorner(0).x;
		// x-position rechte obere ecke
		var x2:Number = getCorner(1).x;
		// abstand
		return Math.abs(x2 - x1);
	}

	/**
	 * gibt hoehe des rechtecks (y-abstand zwischen linker oberer und linker unterer ecke) zurueck
	 * @return gibt hoehe des rechtecks zurueck
	 */
	public function getHeight():Number
	{
		// y-position linke obere ecke
		var y1:Number = getCorner(0).y;
		// y-position linke untere ecke
		var y2:Number = getCorner(2).y;
		// abstand

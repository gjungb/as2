...
class com.adgamewonderland.agw.racing.vehicle.VehicleTest extends Vehicle {
	...
	private static var STEER_ANGLE:Number = 2 / 180 * Math.PI; // lenkwinkel je update
	
	private static var STEER_TIME:Number = 500; // lenkzeit bis erreichen lenkwinkel
	...
	public function onKeyDown():Void
	{
		...
		if (Key.getCode() == KEYS.up && myAccelerationStatus == false) {
			...
			// beschleunigung in entgegengesetzter richtung zur reibung
			acceleration.setAngle(myFriction.getAngle() + Math.PI);
		}
	}
	...
	public function onKeyUp():Void
	{
		...
		if (Key.getCode() == KEYS.up) {
			trace("kein gas geben");
			// status beim beschleunigen
			myAccelerationStatus = false;
			// beschleunigung minimieren => fahrzeug wird sofort langsamer
		...
		// lenken beendet
		if (Key.getCode() == KEYS.left || Key.getCode() == KEYS.right) updateAngles();
	}
	...
	private function updateAcceleration():Void
	{
		...
		// lenkwinkel
		var sangle:Number = onSteer(dirsteer);  // 1 / 180 * Math.PI;

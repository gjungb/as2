...
class com.adgamewonderland.trias.vkftool.reports.Report4 extends Report {
	...
	private var menue_mc:MovieClip;
	
	private var table_supplier_mc:MovieClip;
	
	private var table_games_mc:MovieClip;
	
	private var tablenavi_mc:MovieClip;
	...
	public function Report4()
	{
		...
		myNumLines = 15;
		// testen, ob direkt zu supplier verlinkt
		var selected:Number = _global.Reports.selectedsupplierid;
		// anzeigen und button initialisieren
		if (selected != null) {
			// button zurueck (zu report 8)
			back_btn.onRelease = function () {
				_global.Reports.showReport(8);
			};
			// supplier anzeigen
			showSupplier(selected);
		} else {
			// button ausblenden
			back_btn._visible = false;
		}
		// resetten
		_global.Reports.selectedsupplierid = null;
	}
	...
	public function swapTable(page:Number, mask:Boolean ):Void
	{
		...
		for (var i:Number = 0; i < games.length; i++) {
			// position (seit 2005 in csv als mmc-ranking)
